...
//LCD的画笔颜色和背景色	   
u16 POINT_COLOR=0x0000;	//画笔颜色 也就是字的颜色 RGB565的颜色格式，0x0000指的黑色，0xFFFF指的白色
...
//开始写GRAM，往某个点写入颜色：开始写
void LCD_WriteRAM_Prepare(void)
{
 	LCD->LCD_REG=lcddev.wramcmd;	  
}	 
//LCD写入GRAM，往某个点写入颜色：写入
//RGB_Code:颜色值
...
//画点，想要在液晶屏上画个点
//使用的时候，除了给x，y坐标，需要在函数前赋值颜色，使用POINT_COLOR
//x,y:坐标
//POINT_COLOR:此点的颜色
...
//快速画点,对比上一个函数，多了一个颜色的参数。此函数用的比较多
//x,y:坐标
//color:颜色
...
//SSD1963 背光设置，通过设置PWM调整背光。我们液晶屏不需要设置背光，所以这个函数了解即可
//pwm:背光等级,0~100.越大越亮.
...
//设置窗口,并自动设置画点坐标到窗口左上角(sx,sy).开辟一块区域，矩形区域
//sx,sy:窗口起始坐标(左上角)
...
//初始化lcd，对时钟、IO口、FSMC、等等进行初始化，对ILI9341的驱动。管脚和原理图对应上
...
以下是常用的应用函数
*****************************************************************************************/
//清屏函数：之前屏上有显示的内容，用这个函数可以把之前的内容清除掉
//color:要清屏的填充色
...
//在指定位置显示一个字符
//x,y:起始坐标
//num:要显示的字符:" "--->"~" ：字符在字库里的位置
//size:字体大小 12*6/16*8/24*12，如果觉得字不够大，需要在字库里自行添加
//mode:叠加方式(1)还是非叠加方式(0)：叠加方式是字符的背景和大背景色相同，非叠加方式是字体下面背景色和大背景色不同，有自己的底色
//字库：font.h 可以自己添加字库
...
/*显示数字,高位为0,则不显示：写数字，也可以利用写字符的函数完成，但是这个函数的好处是可以直接把数字做为输入参数
  写入，比如num写入123，可以直接显示123.如果用LCD_ShowChar，需要分别显示1、2、3，然后合到一起。*/
//x,y :起点坐标	 
//len :数字的位数
//size:字体大小
//color:颜色 
...
{  							  
	...
	u16 t;	
	u16 y0=y;
	u16 csize=(size/8+((size%8)?1:0))*(size);		//得到字体一个汉字对应点阵集所占的字节数	

	for(t=0;t<csize;t++)
	{   
		if(size==16)temp=Chinese_16x16[(num-1)*32+t]; 	 	//调用16*16字体
		else if(size==32)temp=Chinese_32x32[(num-1)*128+t];	//调用32*32字体
		else if(size==64)temp=Chinese_64x64[(num-1)*512+t];	//调用64*64字体
		else return;								//没有的字库
		for(t1=0;t1<8;t1++)
		{			    
			if(temp&0x80)LCD_Fast_DrawPoint(x,y,POINT_COLOR);
